...
/* ---------------- Header: translucent on first scroll + active nav spy ---------------- */
  ...
  const onScroll = () => {
    ...
    if (atBottom) {
      ...
      sections.forEach(sec => {
        ...
        if (window.scrollY + (window.innerHeight * 0.38) >= top) {
          currentId = sec.id;
        }
      });
...
function initCircuitTransition() {
  ...
  // Intercept all links targeting hardware-lab.html
  document.addEventListener('click', (e) => {
    ...
    // Match links targeting hardware-lab.html
    const isHardwareLink = href.startsWith('hardware-lab.html') || href.includes('hardware-lab.html');
    ...
    if (stage) {
      ...
      // Fallback to link center if coordinates are 0 (e.g. keyboard accessibility)
      if (typeof clickX !== 'number' || clickX === 0) {
    ...
    // Reset traces so animation starts freshly from click location
    traces.forEach(path => {
    ...
    // Quick micro-transition: active scrim & crisp trace draw-in from click point
    overlay.classList.add('is-active');
