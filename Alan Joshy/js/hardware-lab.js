...
function preloadBuildImages() {
  Object.values(BUILDS).forEach(b => {
    if (b.image) {
      const img = new Image();
      img.src = b.image.replace('.jpg', '.webp');
    }
  });
}

function initBuildEngine() {
  // 1. Determine target build immediately (avoid flashing default 'ambient')
  currentBuildKey = getInitialBuildKey();

  const dockBtns = document.querySelectorAll('.dock-btn');
  dockBtns.forEach(btn => {
    btn.classList.toggle('is-active', btn.getAttribute('data-build') === currentBuildKey);
  ...
  // 2. Render initial target immediately without flash
...
function switchBuild(key) {
  ...
  newUrl.searchParams.set('build', key);
  window.history.replaceState({}, '', newUrl);

  // Update Dock UI active state
  ...
  // Smoothly fade out current card
  card.style.opacity = '0';
  card.style.transform = 'scale(0.96) translateY(8px)';
  if (hotspotsLayer) hotspotsLayer.style.opacity = '0';

  const data = BUILDS[key];

  // Pre-decode new image before revealing card to prevent visual glitch
  const imgLoader = new Image();
  imgLoader.src = data.image;

  const revealNewBuild = () => {
    renderBuild(key);
    requestAnimationFrame(() => {
      card.style.opacity = '1';
      card.style.transform = 'scale(1) translateY(0)';
      if (hotspotsLayer) hotspotsLayer.style.opacity = '1';
      triggerSweep();
    });
  };

  if (imgLoader.complete) {
    setTimeout(revealNewBuild, 150);
  } else {
    imgLoader.onload = () => setTimeout(revealNewBuild, 50);
    imgLoader.onerror = () => setTimeout(revealNewBuild, 150);
  }
}
...
function renderBuild(key) {
  ...
  const mainSource = document.getElementById('mainSourceWebp');
  if (mainImg) {
    mainImg.src = data.image;
    mainImg.alt = data.title;
  }
  if (mainSource) {
    mainSource.srcset = data.image.replace('.jpg', '.webp');
  }
