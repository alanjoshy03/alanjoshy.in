...
function initModal() {
  ...
  // Event delegation for read-more buttons (supports cloned cards)
...
/* ---------------- Journal carousel (Infinite Smooth Auto-Scroll & Interaction Pause) ---------------- */
function initJournalCarousel() {
  ...
  // Duplicate cards for seamless infinite right-to-left scrolling
  ...
  const speed = 0.28; // Calm, gentle, easy-to-read drift pace
  ...
  if (prevBtn) {
    prevBtn.addEventListener('click', () => {
      ...
      setTimeout(() => { scrollPos = track.scrollLeft; }, 400);
    });
  }
  ...
  // Pause on hover
  wrap.addEventListener('mouseenter', () => pauseAutoScroll());
  wrap.addEventListener('mouseleave', () => resumeAutoScroll());

  // Pause on touch or pointer interactions
  wrap.addEventListener('touchstart', () => pauseAutoScroll(), { passive: true });
  wrap.addEventListener('touchend', () => pauseAutoScroll(3000));
  wrap.addEventListener('pointerdown', () => pauseAutoScroll());
  wrap.addEventListener('pointerup', () => pauseAutoScroll(3000));

  // Pause when any card is clicked
  track.addEventListener('click', () => pauseAutoScroll(4000));

  // Animation Frame Loop for continuous, gentle right-to-left scroll
